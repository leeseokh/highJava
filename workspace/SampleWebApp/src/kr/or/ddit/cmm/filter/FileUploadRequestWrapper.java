...
public class FileUploadRequestWrapper extends HttpServletRequestWrapper {
    ...
    private void parsing(HttpServletRequest request,
        ...
        if (ServletFileUpload.isMultipartContent(request)) {
            ...
            addTo(); // 현재 객체를 속성값으로 설정한다.(래퍼클래스 여부 체크를 위해 설정) 멀티파트일때만 실행된다.
        }
    }
    ...
    public String getParameter(String name) {
        if (multipart) {							//멀티파트는 새로 생성한 메서드 / 멀티파트인 경우 파싱
            String[] values = (String[])parameterMap.get(name);
            if (values == null) return null;
            return values[0];
        } else
            return super.getParameter(name);		//super요소 wrapper자식 기존 있던 메서드 사용
    }
     ...
     */
    public void addTo() {	//	멀티파트일때만 실행된다.
