...
public class MemberIbatisTest {
	public static void main(String[] args) {
		...
		try {
			//1-1. xml 문서 읽어오기
			Charset charset =Charset.forName("UTF-8"); //설정 파일 온키동
			...
			mv.setMemAddr("대전광역시 중구 선화동");
			
			//2) SqlMapClient객체 변수를 이용하여 해당 쿼리문을 실행한다.
			//형식 smc.insert("namespace값.id값", 파라미터클래스);
			//  반환값: 성공하면 null이 반환된다.
			
		/*	Object obj = smc.insert("memberTest.insertMember", mv);  //>> namespace로 속성
			if(obj == null) {
