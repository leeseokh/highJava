...
public class T14_ThreadShareDataTest {
	 ...
	 * 
	 * 예제))원주율을 계산하는 쓰레드가 있고, 계산된 원주율을 출력하는 쓰레드가 있다. 원주율을 계산한 후 이 값을 출력하는 프로그램을
...
class ShareData {
	...
	/*
	 * volatile =>선언된 변수를 컴파일러의 최적화 대상에서 제외 시킨다. 즉, 값이 변경되는 즉시 변수에 적용시킨다. 다중 쓰레드에서
	 * 하나의 변수가 완벽하게 한번에 작동되도록 보장하는 키워드 (일종의 동기화)
	 */
	// 원주율 계산이 완료 되었는지 나타내는 변수
