...
public class T07_EnumTest {
	 ...
	 * 열거형 ㅡ> 상수값들을 선언하는 방법
	 	...
	 	열거형 선언하는 방법
	1. 	enum 열거형 이름 { 상수값1, 상수값2, ..., 상수값n}
	 */
		//City 열거형 객체 선언(기본값을 이용하는 열거형)
	public enum City {서울, 부산, 대구, 광주, 대전} // City타입이란 enum 타입	
	...
	public enum Season {
		...
		//괄호속의 값이 저장될 변수 선언
		private String str;		//객체를 막 생성 못함. 생성자가 private..
			
		//생성자 만들기(열거형의 생성자는 제어자가 묵시적으로 'private' 이다.)
		Season(String data) { // <=같다  private Season(String data){
			this.str = data;
		}
		//값을 반환하는 메서드
		public String geStr() {
	...
	public static void main(String[] args) {
		/*
		  	...
		  	2.ordinal() => 열거형 상수가 정의된 순서값을 반환한다. (기본적으로 0부터시작)  ex) 서울 대전 대구 부산 광주
	...
	System.out.println("get메서드=>" + ss.geStr());
	System.out.println("-----------------------------------------");
	
	//열거형이름.values()=> 데이터를 배열로 가져오기
	Season[] enumArr = Season.values();
	for(int i=0; i<enumArr.length; i++) {
		System.out.println(enumArr[i].name() + ":"+ enumArr[i].geStr());
	}
	System.out.println();
