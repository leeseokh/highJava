	...
	class ServerReceiver extends Thread{
		...
		public void run() {
			try {
				...
				name = dis.readUTF();
				//대화명을 받아서 다른 모든 클라이언ㅇ트에게 대화방 참여 메세지를 보낸다.
				sendMessage("#" + name + "님이 입장 했습니다.");
				
				//대화명과 소켓 정보를 Map에 저장한다.
				clients.put(name, socket);
				
				System.out.println("현재 서버 접속자 수는" + clients.size()+ "명 입니다.");
				//이후의 메시지 처리는 반복문으로 처리
				//한 클라이언트가 보낸 메시지를 다른 모든 클라이언트 에게 보내준다.
				while(dis !=null) {
				...
				//이 finally영역이 실행 된다는 것은 클라이언트의 접속이 종료 되었음을 의미
